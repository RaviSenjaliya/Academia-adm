...
export default function NotificationsPopover() {
  ...
  useEffect(() => {
    axios.get('https://academia-api-cu1m.onrender.com/api/students').then((r) => {
  ...
  const handleEditClick = (row) => {
    axios.get(`https://academia-api-cu1m.onrender.com/api/students/${row.row._id}`).then((r) => {
      setEdit(row.row._id);
      setdata({
        name: r.data.name,
        phone: r.data.phone,
  ...
  const handleDeleteClick = (row) => {
    if (window.confirm('Are you sure you want to delete?')) {
      axios.delete(`https://academia-api-cu1m.onrender.com/api/students/${row.row._id}`).then(() => {
  ...
  const handleSubmit = (e) => {
    ...
    e.preventDefault();
    axios.post('https://academia-api-cu1m.onrender.com/api/students', formdata).then((r) => {
      console.log(r.data);
      setEdit(r.data._id);
    });
    setdata({
