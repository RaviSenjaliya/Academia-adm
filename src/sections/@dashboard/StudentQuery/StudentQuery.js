...
export default function StudentQuery() {
  ...
  const handleDeleteClick = (row) => () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://academia-api-cu1m.onrender.com/api/studentquery/${row.row._id}`).then((r) => {
  ...
  useEffect(() => {
    axios.get('https://academia-api-cu1m.onrender.com/api/studentquery').then((r) => {
