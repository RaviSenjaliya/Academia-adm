...
const Session = (props) => {
  ...
  useEffect(() => {
    axios.get('https://academia-api-cu1m.onrender.com/api/session').then((r) => {
      setshow(r.data);
    });
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (data.session) {
        ...
        const response = await axios.put(`https://academia-api-cu1m.onrender.com/api/session/${data._id}`, data);
        toast.success('Data updated successfully');
      } else {
        // If data.session is empty, make a POST request
        const response = await axios.post('https://academia-api-cu1m.onrender.com/api/session', data);
